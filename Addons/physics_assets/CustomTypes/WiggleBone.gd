...
func set_properties(value: WiggleProperties) -> void:
	...
	# set mode as meta here
	if properties.Mode.DISLOCATION == WiggleProperties.Mode.DISLOCATION:
		self.set_meta("mode", "dislocation")
	else:
		self.set_meta("mode", "rotation")
...
func _enter_tree() -> void:
	_skeleton = PAHelpers.get_parent_of_class(self, "Skeleton") as Skeleton
	_fetch_bone()

func _exit_tree() -> void:
	# reset custom pose on node removal
