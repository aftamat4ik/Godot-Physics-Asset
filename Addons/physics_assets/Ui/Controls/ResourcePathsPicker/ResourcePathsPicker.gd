...
var open_resource_dialog: FileDialog
# this folder will be opeded by FileDialog
...
func _ready():
	...
	open_resource_dialog = FileDialog.new()
	
	open_resource_dialog.mode = FileDialog.MODE_OPEN_FILE
	open_resource_dialog.access = FileDialog.ACCESS_RESOURCES
	open_resource_dialog.filters = ["*.tres ; Resources"]
	open_resource_dialog.rect_min_size.x = 550
	open_resource_dialog.rect_min_size.y = 300
	open_resource_dialog.size_flags_horizontal = SIZE_EXPAND_FILL
	open_resource_dialog.size_flags_vertical = SIZE_EXPAND_FILL
	...
	if not root.is_a_parent_of(open_resource_dialog):
		# add dialog to scene
		root.add_child(open_resource_dialog)
		open_resource_dialog.set_owner(_edited_scene_root)
	update_items()
...
func _select_node_path(property_index:int):
	current_item_index = property_index
	# onready vars should be loaded first
	if not is_inside_tree() or _plugin == null:
		return
	
	# set current directory
	open_resource_dialog.set_current_path(resources_folder)
	
	# show dialog
	open_resource_dialog.popup_centered()
	if open_resource_dialog.is_connected("file_selected", self, "_resource_selected"):
		# disconnect from previous
		open_resource_dialog.disconnect("file_selected", self, "_resource_selected")
	open_resource_dialog.connect("file_selected", self, "_resource_selected", [property_index])

# once scene node selected
func _resource_selected(file_path:String, property_index:int):
	if file_path == "":
		return
	...
	items[property_index] = file_path
	emit_signal("updated", items)
